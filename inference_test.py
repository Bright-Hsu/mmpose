...
class Configurator(object):
    def __init__(self):
        self.port = 6010
        self.configSocket = None
        self.configCleintAddress = None

        self.s_dim = [8, 8]
        self.a_dim = 125
        self.state = torch.ones(1, 8, 8)
        self.state[0, 0] = 5
        self.state[0, 1] = 0
        self.state[0, 2] = 1
        self.state[0, 3] = 10
    ...
    def getAction(self, bandwidth, buffer_size, trans_time, video_chunk_size, action, content_change_rate = 0.0):
        # state input
        self.state[:, :, :-1] = self.state[:, :, 1:]
        self.state[0, 0, -1] = bandwidth
        self.state[0, 1, -1] = buffer_size
        self.state[0, 2, -1] = trans_time
        self.state[0, 3, -1] = video_chunk_size

        fps, res, qp = DECODE_PARAMETER_DICT[action]
        self.state[0, 4, -1] = res
        self.state[0, 5, -1] = fps
        self.state[0, 6, -1] = qp
        self.state[0, 7, -1] = content_change_rate
        print("state: ", self.state)
        
        # prediction, output action
        logits = self.agent(torch.FloatTensor(self.state))
        prediction = torch.argmax(logits, dim=-1)
        print("prediction: ", prediction)
        return prediction.item()

    def recvByCount(self, sock, count):  # 读取count长度的数据
        buf = b''
        while count:
    ...
    def videoProcess(self, sock, addr):
        ...
        while not getattr(sock, '_closed'):
            ...
            size_bytes = self.recvByCount(sock, recvSize)
            ...
            if len(size_bytes) == 0 or size == 0:
                continue
            # 接收图片
            imageData = self.recvByCount(sock, size)
        ...
        new_action = self.pre_action
        content_change_rate = 0.0
        if self.pre_file_size == 0:
            content_change_rate = 0.0
            self.pre_file_size = file_size
        else:
            content_change_rate = (file_size - self.pre_file_size) / self.pre_file_size
            self.pre_file_size = file_size
        action = self.getAction(bandwidth, 0, trans_time, video_chunk_size, new_action, content_change_rate)
    ...
    def main(self):
        ...
        server.bind(address)
        server.listen(5)
        print(f"[*]VideoReceiver Listening:  {address}")

        configServerThread = threading.Thread(target=self.configServerInit)
        configServerThread.start()

        while True:
            sock, addr = server.accept()
            # 创建新线程来处理TCP连接:
            t = threading.Thread(target=self.videoProcess, args=(sock, addr))
            t.start()
            t.join()
